...
export default function RegisterForm({ redirect = '/' }: RegisterFormProps) {
  ...
  const onSubmit = async (data: RegisterSchemaType) => {
    ...
    if (result?.success && result.redirectTo && result.phone && result.password) {
      ...
      // Admin-specific message with guide button
      if (result.isAdmin) {
        const swalResult = await Swal.fire({
          title: '🎉 تم إنشاء حسابك بنجاح كمدير!',
          html: `
            <div style="font-size:1.1rem;line-height:2;text-align:right;direction:rtl">
              <div style="margin-bottom:1rem">لبدء إعداد متجرك، يرجى:</div>
              <ol style="margin:1rem 0;padding-right:1.5rem;text-align:right">
                <li>انقر على أيقونة لوحة التحكم <span style="display:inline-flex;align-items:center;justify-content:center;padding:8px;border-radius:8px;background-color:#dc2626;color:white;margin:0 4px;vertical-align:middle">🎯</span> (المربع الأحمر مع أيقونة التركيز) في أعلى الصفحة</li>
                <li>إضافة البيانات الأساسية للمتجر</li>
                <li>إضافة التصنيفات والمنتجات</li>
                <li>تكوين الإعدادات المهمة</li>
              </ol>
            </div>
          `,
          icon: 'success',
