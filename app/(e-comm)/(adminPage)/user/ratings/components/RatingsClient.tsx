...
import { Star, Calendar, Package, ExternalLink, Smartphone, Plus } from 'lucide-react';
...
function UnifiedRatingsStatistics({ productReviews, appRatings }: {
    productReviews: any[],
    appRatings: any[]
}) {
    const totalRatings = productReviews.length + appRatings.length;
...
export default function RatingsClient({
    ...
    const handleAddRating = (type: 'product' | 'app') => {
        switch (type) {
            case 'product':
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-muted-foreground mt-1">إدارة وعرض تقييماتك للمنتجات والمتجر</p>
                        </div>
                    </div>

                    <div className="flex items-center gap-2">
                        <Badge variant="secondary" className="bg-feature-analytics/10 text-feature-analytics border-feature-analytics/20">
                            {productReviews.length + appRatings.length} تقييم إجمالي
                        </Badge>
                    </div>
                </div>
            ...
            <Tabs defaultValue="all" className="w-full">
                ...
                <TabsContent value="all" className="space-y-6">
                    <div className="flex items-center justify-between">
                        <h2 className="text-xl font-semibold">جميع التقييمات ({productReviews.length + appRatings.length})</h2>
                        <div className="text-sm text-muted-foreground">
                    ...
                    <div className="space-y-4">
                        ...
                        {appRatings.map((rating: any, index: number) => (
                            <AppRatingCard key={`app-${rating.id}`} rating={rating} index={index + productReviews.length} />
                        ))}

                        {/* Empty state if no ratings */}
                        {productReviews.length === 0 && appRatings.length === 0 && (
